...
function Library.setTvSource(tvIp, tvPort, source)
    local runcommand = 'curl -X POST -H "Content-Type: application/json" -d \'{"id":"' .. source .. '"}\' http://' .. tvIp .. ':' .. tvPort .. '/6/sources/current'
    os.execute(runcommand)
end
